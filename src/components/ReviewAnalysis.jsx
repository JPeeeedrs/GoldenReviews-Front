...
export default function ReviewAnalysis({ data }) {
  ...
  // O Polling ABSA JSON alterou a estrutura 'topics'
  // Agora vem dividida direto entre "positive" e "negative" (arrays)
  const orderedTopics = useMemo(() => {
    if (!data?.topics) return [];

    const topicsArray = data.topics[topicView] || [];
    // Eles ja vem ordenados por mencoes da API ABSA
    return topicsArray.slice(0, 10);
  }, [data?.topics, topicView]);

  if (!data) return null;

  // Tratar dados se ainda estiver renderizando tela vazia (ex: inicio do poll que passou pelo loading)
  ...
  return (
    <section className="analysis">
      <header className="analysis-hero">
        <div>
          <p className="eyebrow">Golden Reviews ABSA • jogo em análise</p>
          <h2>{game?.name ?? ""}</h2>
      ...
      <div className="stat-grid">
        <div className="stat-card">
          ...
          <strong>{numberFormat(summary?.reviews_analyzed ?? 0)}</strong>
        </div>
        <div className="stat-card neutral">
          <span>Processamento Assíncrono</span>
          <strong>
            {data.metadata?.processing_time_seconds
              ? `${data.metadata.processing_time_seconds}s`
              : "Cache"}
            &nbsp;•&nbsp;
            {data.metadata?.model_version || "BERTopic"}
          </strong>
        </div>
        {owners && (
          ...
          return (
            <div className={`topic-card ${topicView}`} key={`topic-${idx}`}>
              ...
              <div className="topic-columns">
                <div>
                  <p className="eyebrow">Score: {topic.score} / 5.0</p>
                  <p
                    className="eyebrow"
